...
plot(density(dat$lifeExp))
# Plot the density object, bandwidth of 10
plot(density(dat$lifeExp, bw = .5, na.rm=T))
```
...
```{r}
ggplot(data = dat, aes(x=year, y=lifeExp, group=country, color=continent)) + geom_line()
```
...
```{r}
ggplot(data = dat, aes(x=year, y=lifeExp, group=country, color=continent)) + geom_line() + geom_point()
```
...
```{r}
ggplot(data = dat, aes(x=year, y=lifeExp, group=country)) + geom_line(aes(color=continent)) + geom_point()
```
